...
`publisher-exact` 的內容寬度是 13.00 cm；`publisher-narrow` 是 15.06 cm。兩者不可能同時維持完全相同的換行、分頁與總頁數。`exact` 表示幾何與樣式契約，不代表任何不同來源、不同字型或不同 renderer 都能產生逐頁相同的像素結果。
...
- `balanced`：四邊 2.00 cm。
- `standard`：四邊 2.54 cm。
- `publisher-exact`：上下 2.10 cm、左右 2.30 cm。
...
- 頁首留空，頁尾只保留置中頁碼。
- 正文使用 11pt、1.25 倍行距、段後 6pt。
- 對話框的角色名稱獨立一行，內容從下一行開始；段前後各 20pt、行距 1.15。
- 一般清單段後 4pt；表格儲存格使用 10pt、段後 3pt、行距 1.15。
